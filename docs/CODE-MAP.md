...
| Print-bed grid overlay (polar vs rectangle) | `Core/Models/BedBoundaryOverlay.cs` → `ViewportView.ApplyActiveBedBoundary` (cell swap, KRL BASE change, `RebuildBed` / `RebuildBedGridSize`). Drawn in Arctic/Preview/Body; only the 2D slice viewer skips it. Dual-bed ghosting: `BaseBedGhosting` + `ViewportView.ApplyBaseBedGhosting`. |
