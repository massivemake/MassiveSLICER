...
public class BeadSupportCheckTest
{
    ...
    /// <summary>
    /// A wipe is Extrude-kind, but the KRL export runs it extruder-off (RPM 0, suck-back), so it
    /// lays no bead. Counted as bead, every ~35 mm wipe off a contour end — which heads off the
    /// part, nothing under it — read as a failing stretch. It must be treated like a travel:
    /// never measured, and a break in the run rather than a continuation of it.
    /// </summary>
    [Fact]
    public void A_wipe_is_not_bead_so_it_neither_fails_nor_joins_a_run()
    {
        var below = Layer(0, 0f);
        Seg(below, 0f, 0f, 100f, 0f);

        var above = Layer(1, 3f);
        Seg(above, 0f, 4f, 8f, 4f);                                  // 8 mm bad, bridgeable
        above.Moves.Add(new ToolpathMove(                             // 35 mm wipe out into air
            new Vector3(8, 4, 3), new Vector3(8, 39, 3), MoveKind.Extrude) { IsWipe = true });
        Seg(above, 8f, 4f, 16f, 4f);                                 // another 8 mm bad
        ...
        Assert.Equal(BeadSupport.SupportVerdict.NotMeasured, r.Verdict[2]);
        Assert.True(float.IsNaN(r.OffsetMm[2]), "a wipe must not be measured at all");
        // Counted, the wipe alone is a 35 mm failure; welded into the run it is 51 mm.
        Assert.False(r.HasFailures);
        Assert.Equal(2, r.Verdict.Count(v => v == BeadSupport.SupportVerdict.Bridged));
        Assert.Equal(16f, r.TotalExtrudedMm - 100f, 2);             // wipe length not in the total
    }

    /// <summary>
    /// A wipe lays nothing, so it cannot hold up the layer above either. A bead sitting over a
    /// wipe and nothing else has no support under it.
    /// </summary>
    [Fact]
    public void A_wipe_in_the_layer_below_does_not_count_as_support()
    {
        var below = Layer(0, 0f);
        Seg(below, 0f, 0f, 50f, 0f);
        below.Moves.Add(new ToolpathMove(
            new Vector3(50, 0, 0), new Vector3(50, 40, 0), MoveKind.Extrude) { IsWipe = true });

        var above = Layer(1, 3f);
        Seg(above, 50f, 10f, 50f, 40f);                              // right on top of the wipe
        ...
        Assert.Equal(BeadSupport.SupportVerdict.Failed, r.Verdict[2]);
    }

    /// <summary>
    /// The slicer's rescale aims to land exactly ON target, so float rounding leaves a thinned
    /// layer a hair past it. The check must give the same 0.1 % slack the slicer re-measures
    /// with, or every successful correction shows up as a miss.
    /// </summary>
    [Fact]
    public void A_bead_a_rounding_hair_past_target_still_passes()
    {
        var tp = Stack(0f, Target * 1.0005f);
