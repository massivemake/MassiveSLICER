...
/// </summary>
/// <remarks>
/// Every assertion on decisions reads the walk's OWN <c>out</c> decisions, never the shared
/// <see cref="SupportDrivenLayerHeights.LastDecisions"/> static. xUnit runs test classes in
/// parallel and every planar slice with the feature off resets that static, so asserting on it
/// passes when filtered and fails at random in the full suite.
/// </remarks>
public class SupportDrivenLayerHeightTest
{
    ...
    {
        var z = SupportDrivenLayerHeights.ThinForSupport(
            Ladder(11, 3f, MaxH), 30f, Wall(0f), Target, Tol, MinH, MaxH, Bead, out var d);

        Assert.All(Heights(z), h => Assert.Equal(MaxH, h, 3));
        Assert.All(d, x => Assert.False(x.Thinned));
    }
    ...
    {
        var z = SupportDrivenLayerHeights.ThinForSupport(
            Ladder(11, 3f, MaxH), 30f, Wall(1f), Target, Tol, MinH, MaxH, Bead, out var d);

        var h = Heights(z);
        Assert.NotEmpty(h);
        Assert.All(h, v => Assert.Equal(Target, v, 2));      // 3 x 2.4/3.0 = 2.4
        Assert.Contains(d, x => x.Thinned);
        Assert.All(d, x => Assert.False(x.Unfixable));
    }
    ...
    {
        var z = SupportDrivenLayerHeights.ThinForSupport(
            Ladder(11, 3f, MaxH), 30f, Wall(2f), Target, Tol, MinH, MaxH, Bead, out var d);

        Assert.All(Heights(z), v => Assert.Equal(MinH, v, 3));
        Assert.All(d, x => Assert.True(x.Unfixable));
        var first = d[0];
    ...
    public void A_short_off_target_stretch_is_bridged_but_a_long_one_thins_the_layer()
    {
        ...
        var bridged = SupportDrivenLayerHeights.ThinForSupport(
        ...
        var thinned = SupportDrivenLayerHeights.ThinForSupport(
            Ladder(6, 3f, MaxH), 15f, notched, Target, bridgeToleranceMm: 2f,
            minLayerHeight: MinH, maxLayerHeight: MaxH, searchCellMm: Bead, decisions: out var d);
        Assert.Contains(d, x => x.Thinned);
    ...
    {
        var one = new[] { 3f };
        Assert.Same(one, SupportDrivenLayerHeights.ThinForSupport(
            one, 30f, Wall(1f), Target, Tol, MinH, MaxH, Bead));

        var ladder = Ladder(5, 3f, MaxH);
        Assert.Same(ladder, SupportDrivenLayerHeights.ThinForSupport(
