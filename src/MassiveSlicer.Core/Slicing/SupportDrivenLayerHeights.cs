...
public static class SupportDrivenLayerHeights
{
    ...
    /// <summary>Decisions from the most recent completed <see cref="ThinForSupport(float[], float, Func{float, IReadOnlyList{IReadOnlyList{Vector2}}}, float, float, float, float, float, bool)"/>, published atomically.</summary>
    ...
    /// <param name="recordDecisions">
    /// Publish to <see cref="LastDecisions"/>. Only a real slice should; anything else that walks
    /// a ladder must not overwrite what <c>support-height-debug</c> reads.
    /// </param>
    public static float[] ThinForSupport(
        float[] proposed,
        float zMax,
        ...
        => ThinForSupport(proposed, zMax, contoursAt, targetOffsetMm, bridgeToleranceMm,
                          minLayerHeight, maxLayerHeight, searchCellMm, out _, recordDecisions);

    /// <inheritdoc cref="ThinForSupport(float[], float, Func{float, IReadOnlyList{IReadOnlyList{Vector2}}}, float, float, float, float, float, bool)"/>
    /// <param name="decisions">
    /// This walk's own decisions. Read these rather than <see cref="LastDecisions"/> when the
    /// answer must belong to THIS call — the static is shared, so any slice running at the same
    /// time (another test class, say) can replace it underneath you.
    /// </param>
    public static float[] ThinForSupport(
        float[] proposed,
        float zMax,
        ...
        float maxLayerHeight,
        float searchCellMm,
        out IReadOnlyList<Decision> decisions,
        bool  recordDecisions = true)
    {
        ...
        while (z < zMax - 1e-4f && outZ.Count < guard)
        {
            ...
            // Same end rule as the uniform and adaptive ladders: no plane at or above the top.
            // Without it the walk always appended one extra plane at zMax, so switching this on
            // added a layer even to a vertical wall it never thinned.
            if (z + finalH >= zMax - 1e-4f) break;

            var after = Worst(runs, bridgeToleranceMm);
            made.Add(new Decision(
                z, h, finalH,
        ...
        // Only a real slice publishes these; nothing else may overwrite what
        // support-height-debug reads.
        Decision[] result = [.. made];
        decisions = result;
        if (recordDecisions) s_last = result;
        return [.. outZ];
    }
