...
public static class BeadSupport
{
    ...
    /// <list type="bullet">
    /// <item><b>NaN</b> — not measured. A travel or a wipe lays no bead, and the first layer (or any
    ...
    /// <summary>
    /// True when a move actually lays bead. A wipe is Extrude-kind but the export runs it with the
    /// extruder off (RPM 0, suck-back), so it deposits nothing: it is neither a bead to measure
    /// nor material that holds up the layer above. Same rule <c>BeadObstacleGrid</c> uses.
    /// Counted as bead, every ~35 mm wipe off a contour end read as a failing stretch.
    /// </summary>
    private static bool LaysBead(ToolpathMove move)
        => ToolpathMoveKinds.IsCutSegment(move.Kind) && !move.IsWipe;
        ...
        for (int li = 0; li < toolpath.Layers.Count; li++)
        {
            ...
            for (int mi = 0; mi < layer.Moves.Count; mi++, flat++)
            {
                ...
                if (!LaysBead(move))
                {
                    ...
                    // continuation of the stretch before it, and neither is a wipe, which runs
                    // with the extruder off. Left NotMeasured: it is never a pass or a failure.
                    CloseRun();
                    continue;
                }
                ...
                if (mm > targetOffsetMm * 1.001f)   // same hair of slack the slicer uses — landing exactly ON target is a pass
                {
                    pastMm += len;
                    runIdx.Add(flat);
                    runTotal += len;
    ...
    /// <c>SupportDrivenLayerHeights.ThinForSupport</c> then computes <c>needed = h * target / inf = 0</c>
