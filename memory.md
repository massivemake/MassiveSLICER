...
1. **PBR polish (core done — see Completed features):** real metallic-roughness PBR with textures now renders. Remaining nice-to-haves: full prefiltered-env + BRDF LUT IBL (v1 uses roughness→LOD + analytic Karis fit); alpha **blend** ordering (v1 = Opaque + Mask only); populate `UvSettingsViewModel` from the selected mesh; later: apply the material system to toolpath meshes + feed the slicer.
   - **"Make it pop" — tried & reverted (2026-06-21):** an in-shader exposure/IBL-gain boost in `MeshRenderer.FragSrc` **grayed** the crystal — it's mostly metal so its colour comes from albedo-tinted env reflection, and brightening pushes it into ACES's desaturation shoulder (post-saturation can't recover). **Don't boost exposure in-shader.** The right lever is a **brighter/colourful HDRI environment**: the IBL samples the scene backdrop, and a dim grey backdrop = dim grey reflections. A library exists in `assets/Images/*.hdr` (AmbienceExposure4k, CasualDay4K, …) selectable via the left-panel **BACKDROP** section (Viewport tab). Open question for the user: pick a brighter default backdrop and/or add a user exposure/IBL slider next to the LIGHTING Intensity slider.
